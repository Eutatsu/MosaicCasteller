...
def importarcsv():
    ...
    with open("Castellers Group Details.csv", encoding="utf-8") as csvf:
        csvReader = csv.DictReader(csvf)  # Reads CSV as dictionary per row
        for row in csvReader:
            data.append(dict(row))  # Convert OrderedDict to a normal dict and append

    for row in data:
        colla = {
            "nom":row["name"],
            "fundacio":row["date_of_foundation"],
            "localitat":row["city_of_origin"],
            "escut":row["logo_link"]
        }
        
        dades_colles.append(colla)

        print(f"Nom Colla: {row["name"]}")      
        print(f"Fundació: {row["date_of_foundation"]}") 
        print(f"Localitat: {row["city_of_origin"]}") 
        print(f"Escut: {row["logo_link"]}") 
...
def combinar_dades():
    ...
    dades_base=input("Importar JSON 1:")

    fitxer_dades_noves=input("Importar JSON 2:")

    try:
        with open(f'{dades_base}', 'r', encoding='utf-8') as json_file:
            dades_colles = json.load(json_file)
    except:
        print("Fitxer 1 erroni")
    try:
        with open(f'{fitxer_dades_noves}', 'r', encoding='utf-8') as json_file:
            dades_noves = json.load(json_file)
    except:
        print("Fitxer 2 erroni")

    for nova_dada in dades_noves:

        key=nova_dada["nom"]
        print(key)
        colla_present = next((colla for colla in dades_colles if colla["nom"]==key), None)
        if colla_present:
            colla_present.update({k: v for k, v in nova_dada.items() if v})
        else:
            print("Colla no inclosa, saltant" )
...
while True:
    ...
    print('[0] Sortir')
    print('[1] Compilar Llista')
    print('[2] Compilar Informació')
    print('[3] Convetir HEX a HSV')
    print('[4] Convetir HEX a HSV')
